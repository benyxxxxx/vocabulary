class KanjisController < ApplicationController
  ...
  def index
    if params[:word_id]
      @kanjis = Word.find(params[:word_id]).kanjis;
    else
      #@kanjis = Kanji.where("learned =  'false'").group(:kanji).order(:id).reverse_order()
     ## @kanjis = Kanji.where("learned =  'false'").order(:id).reverse_order()
      @kanjis = Kanji.joins(:words).group("kanji_id") .order("COUNT(*) DESC")
  ...
  def update
    respond_to do |format|
      if @kanji.update(kanji_params)
        ...
        format.json { render :show, status: :ok, location: @kanjis }
      else
  ...
  private
    ...
    def kanji_params
      params.require(:kanji).permit(:kanji, :due, :learned)
    end
end
